...
这类模式通常是为了解决性能优化或系统解耦。

| 模式 | 是什么 | 什么时候用 |
|------|------|:----:|
| 脏标识 (Dirty Flag) | 只有数据变了才重新计算，否则用缓存。 | 性能优化神器。避免每帧重复计算昂贵的逻辑（如：物理引擎、UI 渲染）。 |
| 服务定位器 (Service Locator) | 建立一个全局的服务超市，要啥找它拿。 | 当系统组件之间的依赖关系复杂时，使用服务定位器可以将依赖关系解耦。 |
