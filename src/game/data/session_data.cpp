...
bool SessionData::loadDefaultData(std::string_view path) {
    ...
    if (!file.is_open()) {
        spdlog::error("not open Session data file: {}", path);
        return false;
    }
    ...
    try {
        ...
        spdlog::error("load Session data failed: {}", e.what());
        return false;
    }
    return true;
}
...
bool SessionData::saveToFile(std::string_view path) {
    ...
    if (!file_path.parent_path().empty() && !std::filesystem::exists(file_path.parent_path())) {
        try {
            ...
            spdlog::error("not create directory {}: {}", file_path.parent_path().string(), e.what());
            return false;
        }
    }
    std::ofstream file(file_path);
    if (!file.is_open()) {
        spdlog::error("not open save file: {}", path);
        return false;
    }
    ...
    file << json.dump(4);
    file.close();
    spdlog::info("save file {} success", path);
    return true;
}
...
void SessionData::removeUnit(entt::id_type name_id) {
    if (auto it = unit_map_.find(name_id); it != unit_map_.end()) {
        ...
        spdlog::error("not found unit: {}", name_id);
    }
}
