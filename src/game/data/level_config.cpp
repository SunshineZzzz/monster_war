...
bool LevelConfig::loadFromFile(std::string_view level_json_path) {
    ...
    if (!file.is_open()) {
        spdlog::error("not open level config file: {}", level_json_path);
        return false;
    }
    ...
    try {
        if (!json.is_array()) {
            spdlog::error("level config file is not array");
            return false;
        }
        ...
        for (const auto& data : json) {
            ...
            spdlog::info("level {} total enemy count: {}", level_data.level_number_, total_enemy_count);
        ...
        spdlog::info("level config load complete");
    } catch (const nlohmann::json::parse_error& e) {
        spdlog::error("parse level config file failed: {}", e.what());
        return false;
    }
    return true;
}
