...
void TitleScene::init() {
    if (!initSessionData())         { spdlog::error("init session_data_ failed"); return; }
    if (!initLevelConfig())         { spdlog::error("init level_config_ failed"); return; }
    if (!initBlueprintManager())    { spdlog::error("init blueprint_manager_ failed"); return; }
    if (!initUIConfig())            { spdlog::error("init ui_config_ failed"); return; }
    if (!loadTitleLevel())          { spdlog::error("load title level failed"); return; }
    if (!initSystems())             { spdlog::error("init systems failed"); return; }
    if (!initRegistryContext())     { spdlog::error("init registry context failed"); return; }
    if (!initUI())                  { spdlog::error("init ui failed"); return; }
