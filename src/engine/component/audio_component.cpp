    ...
    : audio_player_(audio_player), camera_(camera)
{
    if (!audio_player_ || !camera_) {
        spdlog::error("AudioComponent initialization failed: audio player or camera is null.");
    }
}
...
{
    if (!owner_) {
        spdlog::error("AudioComponent: the owner is null! This component will be invalid.");
        return;
    }
    transform_ = owner_->getComponent<TransformComponent>();
    if (!transform_) {
        spdlog::warn("AudioComponent: the owner GameObject '{}' requires a TransformComponent, but none was found.", owner_->getName());
    }
}
...
void AudioComponent::playSound(std::string_view sound_id, int channel, bool use_spatial)
{
    ...
    if (use_spatial && transform_) {    // 使用空间定位
        ...
        if (distance > 150.0f) {
            spdlog::debug("AudioComponent: sound '{}' is out of range, not played.", sound_id);
            return; // 超出范围，不播放
        }
...
void AudioComponent::addSound(std::string_view sound_id, std::string_view sound_path)
{
    if (sound_id_to_path_.find(std::string(sound_id)) != sound_id_to_path_.end()) {
        spdlog::warn("AudioComponent: sound ID '{}' already exists, old path will be overridden.", sound_id);
    }
    sound_id_to_path_[std::string(sound_id)] = sound_path;
    spdlog::debug("AudioComponent: add sound ID '{}' with path '{}'", sound_id, sound_path);
}
