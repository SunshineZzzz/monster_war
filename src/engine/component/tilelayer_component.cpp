      ...
      tiles_(std::move(tiles))
{
    if (tiles_.size() != static_cast<size_t>(map_size_.x * map_size_.y)) {
        spdlog::error("TileLayerComponent: the map size does not match the provided tile vector size. The tile data will be cleared.");
        tiles_.clear();
        map_size_ = {0, 0};
    }
    spdlog::trace("TileLayerComponent constructed successfully.");
}

void TileLayerComponent::init() {
    if (!owner_) {
        spdlog::warn("TileLayerComponent: the owner_ is not set.");
    }
    spdlog::trace("TileLayerComponent initialized successfully.");
}
...
const TileInfo* TileLayerComponent::getTileInfoAt(glm::ivec2 pos) const {
    if (pos.x < 0 || pos.x >= map_size_.x || pos.y < 0 || pos.y >= map_size_.y) {
        spdlog::warn("TileLayerComponent: the tile position ({}, {}) is out of bounds.", pos.x, pos.y);
        return nullptr;
    }
    ...
    spdlog::warn("TileLayerComponent: the tile index {} is out of bounds.", index);
    return nullptr;
}
