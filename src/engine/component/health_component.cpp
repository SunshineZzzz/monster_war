...
bool HealthComponent::takeDamage(int damage_amount) {
    ...
    if (is_invincible_) {
        spdlog::debug("the owner GameObject '{}' is invincible, and took {} damage. This damage is ignored.", owner_ ? owner_->getName() : "Unknown", damage_amount);
    ...
    spdlog::debug("the owner GameObject '{}' took {} damage, and now has {} health out of {}.",
...
int HealthComponent::heal(int heal_amount) {
    ...
    spdlog::debug("the owner GameObject '{}' healed {} health, and now has {} health out of {}.",
...
void HealthComponent::setInvincible(float duration)
{
    if (duration > 0.0f)
    {
        ...
        spdlog::debug("the owner GameObject '{}' is now invincible for {} seconds.", owner_ ? owner_->getName() : "Unknown", duration);
    } else {
        // 如果持续时间为 0 或负数，则立即取消无敌
        is_invincible_ = false;
        invincibility_timer_ = 0.0f;
        spdlog::debug("the owner GameObject '{}' is no longer invincible.", owner_ ? owner_->getName() : "Unknown");
    }
}
