...
{
    if (!collider_) {
        spdlog::error("create ColliderComponent with empty collider!");
    }
}

void ColliderComponent::init() {
    if (!owner_) {
        spdlog::error("ColliderComponent: the owner is null! This component will be invalid.");
        return;
    }
    transform_ = owner_->getComponent<TransformComponent>();
    if (!transform_) {
        spdlog::error("ColliderComponent: the owner GameObject '{}' requires a TransformComponent, but none was found.", owner_->getName());
        return;
    }
