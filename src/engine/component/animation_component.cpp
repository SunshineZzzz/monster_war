...
void AnimationComponent::init() {
    if (!owner_) {
        spdlog::error("AnimationComponent: the owner is null! This component will be invalid.");
        return;
    }
    sprite_component_ = owner_->getComponent<SpriteComponent>();
    if (!sprite_component_) {
        spdlog::error("AnimationComponent: the owner GameObject '{}' requires a SpriteComponent, but none was found.", owner_->getName());
        return;
    }
}
...
void AnimationComponent::update(float delta_time, engine::core::Context&) {
    ...
    if (!is_playing_ || !current_animation_ || !sprite_component_ || current_animation_->isEmpty()) {
        spdlog::trace("AnimationComponent: no animation is playing or sprite component is null or animation is empty.");
        return;
    }
...
void AnimationComponent::addAnimation(std::unique_ptr<engine::render::Animation> animation) {
    ...
    spdlog::debug("AnimationComponent: added animation '{}' to GameObject '{}'", name, owner_ ? owner_->getName() : "Unknown");
}
...
void AnimationComponent::playAnimation(std::string_view name) {
    ...
    if (it == animations_.end() || !it->second) {
        spdlog::warn("AnimationComponent: failed to find animation '{}' for GameObject '{}'", name, owner_ ? owner_->getName() : "Unknown");
        return;
    }
