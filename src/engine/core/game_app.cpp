...
void GameApp::handleEvents() {
    if (input_manager_->shouldQuit()) {
        spdlog::trace("GameApp received quit request from InputManager.");
        is_running_ = false;
        return;
    }
...
bool GameApp::initPhysicsEngine()
{
    try {
        ...
        spdlog::error("initialize physics engine failed: {}", e.what());
        return false;
    }
    spdlog::trace("physics engine initialized successfully.");
    return true;
}
