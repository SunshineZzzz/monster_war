...
Time::Time() {
    ...
    spdlog::trace("Time initialized. Last time: {}", last_time_);
}

void Time::update() {
    frame_start_time_ = SDL_GetTicksNS();   // 记录进入 update 时的时间戳
    float current_delta_time = static_cast<float>(frame_start_time_ - last_time_) / 1000000000.0;
...
void Time::setTimeScale(float scale) {
    if (scale < 0.0) {
        spdlog::warn("Time scale cannot be negative. Clamping to 0.");
        scale = 0.0; // 防止负时间缩放
    }
    time_scale_ = scale;
}
...
void Time::setTargetFps(int fps) {
    if (fps < 0) {
        spdlog::warn("Target FPS cannot be negative. Setting to 0 (unlimited).");
        target_fps_ = 0;
    } else {
        target_fps_ = fps;
    }
