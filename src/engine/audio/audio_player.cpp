...
int AudioPlayer::playSound(std::string_view sound_path, int channel) {
    ...
    if (!chunk) {
        spdlog::error("AudioPlayer: failed to get sound '{}' for playback.", sound_path);
        return -1;
    }

    int played_channel = Mix_PlayChannel(channel, chunk, 0);    // 播放音效
    if (played_channel == -1) {
        spdlog::error("AudioPlayer: failed to play sound '{}': {}", sound_path, SDL_GetError());
    } else {
         spdlog::trace("AudioPlayer: play sound '{}' on channel {}.", sound_path, played_channel);
    }
...
bool AudioPlayer::playMusic(std::string_view music_path, int loops, int fade_in_ms) {
    ...
    if (!music) {
        spdlog::error("AudioPlayer: failed to get music '{}' for playback.", music_path);
        return false;
    }
    ...
    if (!result) {
        ...
        spdlog::trace("AudioPlayer: play music '{}'.", music_path);
    }
    return result;
}
