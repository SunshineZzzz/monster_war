...
void InputManager::update() {
    ...
    // 3. 触发回调
    for (auto& [actions_name_id, state] : action_states_) {
        if (state != ActionState::INACTIVE) {
            if (auto it = actions_to_func_.find(actions_name_id); it != actions_to_func_.end()) {
                // 触发回调
...
void InputManager::initializeMappings(const engine::core::Config* config) {
    ...
    if (actions_to_keyname.find("mouse_left") == actions_to_keyname.end()) {
         spdlog::debug("config does not define 'mouse_left' action, add default mapping to 'mouse_left'.");
         actions_to_keyname["mouse_left"] = {"MouseLeft"};     // 如果缺失则添加默认映射
    }
     if (actions_to_keyname.find("mouse_right") == actions_to_keyname.end()) {
         spdlog::debug("config does not define 'mouse_right' action, add default mapping to 'mouse_right'.");
