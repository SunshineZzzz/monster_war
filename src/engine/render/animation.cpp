...
void Animation::addFrame(SDL_FRect source_rect, float duration) {
    if (duration <= 0.0f) {
        spdlog::warn("try add invalid duration frame to animation '{}'", name_);
        return;
    }
...
const AnimationFrame& Animation::getFrame(float time) const {
    if (frames_.empty()) {
        spdlog::error("animation '{}' has no frame, can not get frame", name_);
    ...
    spdlog::warn("animation '{}' get frame info failed, return last frame", name_);
    return frames_.back();
}
