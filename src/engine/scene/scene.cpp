...
void Scene::init() {
    ...
    spdlog::trace("scene '{}' initialized successfully.", scene_name_);
}
...
void Scene::update(float delta_time) {
    ...
    for (auto& obj : game_objects_) {
        if (obj && !obj->isNeedRemove()) {
            ...
            else spdlog::warn("attempt to update null game object pointer.");
        }
    }
...
void Scene::clean() {
    ...
    spdlog::trace("scene '{}' cleaned successfully.", scene_name_);
}
...
void Scene::addGameObject(std::unique_ptr<engine::object::GameObject>&& game_object) {
    ...
    else spdlog::warn("attempt to add null game object pointer to scene '{}'.", scene_name_);
}
...
void Scene::removeGameObject(engine::object::GameObject* game_object_ptr) {
    if (!game_object_ptr) {
        spdlog::warn("attempt to remove null game object pointer from scene '{}'.", scene_name_);
        return;
    }
    ...
    if (it != game_objects_.end()) {
        ...
        spdlog::trace("game object pointer removed successfully from scene '{}'.", scene_name_);
    } else {
