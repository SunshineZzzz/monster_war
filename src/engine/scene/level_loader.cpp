...
bool LevelLoader::loadLevel(std::string_view level_path, Scene& scene) {
    ...
    try {
        ...
        spdlog::error("failed to parse level file '{}': {}", level_path, e.what());
        return false;
    }
    ...
    if (json_data.contains("tilesets") && json_data["tilesets"].is_array()) {
        for (const auto& tileset_json : json_data["tilesets"]) {
            if (!tileset_json.contains("source") || !tileset_json["source"].is_string() ||
                ...
                spdlog::error("tilesets object in level file '{}' is missing valid 'source' or 'firstgid' fields.", level_path);
                continue;
            }
    ...
    if (!json_data.contains("layers") || !json_data["layers"].is_array()) {       // 地图文件中必须有 layers 数组
        spdlog::error("level file '{}' is missing or invalid 'layers' array.", level_path);
        return false;
    }
    for (const auto& layer_json : json_data["layers"]) {
        ...
        if (!layer_json.value("visible", true)) {
            spdlog::info("layer '{}' in level file '{}' is not visible, skip loading.", layer_json.value("name", "Unnamed"), level_path);
            continue;
        }
        ...
        if (layer_type == "imagelayer") {       
            ...
            spdlog::warn("unsupported layer type '{}' in level file '{}', skip loading.", layer_type, level_path);
        }
    }

    spdlog::info("level '{}' loaded successfully.", level_path);
    return true;
}
...
void LevelLoader::loadImageLayer(const nlohmann::json& layer_json, Scene& scene) {
    ...
    std::string image_path = layer_json.value("image", "");     // json.value()返回的是一个临时对象，需要赋值才能保存，
                                                                // 不能用std::string_view
    if (image_path.empty()) {
        spdlog::error("image layer '{}' is missing 'image' property.", layer_json.value("name", "Unnamed"));
        return;
    }
    ...
    spdlog::info("image layer '{}' loaded successfully.", layer_name);
}
...
void LevelLoader::loadTileLayer(const nlohmann::json& layer_json, Scene& scene)
{
    if (!layer_json.contains("data") || !layer_json["data"].is_array()) {
        spdlog::error("tile layer '{}' is missing 'data' property.", layer_json.value("name", "Unnamed"));
        return;
    }
    ...
    // 添加到场景中
    scene.addGameObject(std::move(game_object));
    spdlog::info("tile layer '{}' loaded successfully.", layer_name);
}
...
void LevelLoader::loadObjectLayer(const nlohmann::json& layer_json, Scene& scene)
{
    if (!layer_json.contains("objects") || !layer_json["objects"].is_array()) {
        spdlog::error("object layer '{}' is missing 'objects' property.", layer_json.value("name", "Unnamed"));
        return;
    }
    // 获取对象数据
    ...
    for (const auto& object : objects) {
        ...
        if (gid == 0) {     // 如果gid为0 (即不存在)，则代表自己绘制的形状
            ...
            if (!src_size_opt) {        // 正常情况下，所有瓦片的Sprite都设置了源矩形，没有代表某处出错
                spdlog::error("gid {} has no source rectangle.", gid);
                continue;
            }
            ...
            if (gravity) {
                ...
                if (pc) {
                    ...
                    spdlog::warn("object '{}' has no physics component when setting gravity, please check map settings.", object_name);
            ...
            if (anim_string) {
                ...
                try {
                    ...
                    spdlog::error("parse animation JSON string failed: {}", e.what());
                    continue;  // 跳过此对象
                }
            ...
            if (sound_string) {
                ...
                try {
                    ...
                    spdlog::error("parse sound JSON string failed: {}", e.what());
                    continue;  // 跳过此对象
                }
                // 添加AudioComponent
            ...
            // 添加到场景中
            scene.addGameObject(std::move(game_object));
            spdlog::info("object '{}' loaded successfully.", object_name);
        }
    }
}
...
void LevelLoader::addAnimation(const nlohmann::json& anim_json, engine::component::AnimationComponent *ac, const glm::vec2& sprite_size)
{
    ...
    if (!anim_json.is_object() || !ac) {
        spdlog::error("invalid animation JSON or AnimationComponent pointer.");
        return;
    }
    ...
    for (const auto& anim : anim_json.items()) {
        ...
        if (!anim_info.is_object()) {
            spdlog::warn("animation '{}' info is invalid or empty.", anim_name);
            continue;
        }
        // 获取可能存在的动画帧信息
        ...
        if (!anim_info.contains("frames") || !anim_info["frames"].is_array()) {
            spdlog::warn("animation '{}' missing 'frames' array.", anim_name);
            continue;
        }
        ...
        for (const auto& frame : anim_info["frames"]) {
            if (!frame.is_number_integer()) {
                spdlog::warn("animation '{}' frame index is not integer, skip.", anim_name);
                continue;;
            }
...
void LevelLoader::addSound(const nlohmann::json &sound_json, engine::component::AudioComponent *audio_component)
{
    if (!sound_json.is_object() || !audio_component) {
        spdlog::error("invalid sound JSON or AudioComponent pointer.");
        return;
    }
    ...
    for (const auto& sound : sound_json.items()) {
        ...
        if (sound_id.empty() || sound_path.empty() ) {
            spdlog::warn("sound '{}' missing necessary information.", sound_id);
            continue;
        }
        // 添加音效到 AudioComponent
...
engine::component::TileInfo LevelLoader::getTileInfoByGid(int gid)
{
    ...
    if (tileset_it == tileset_data_.begin()) {
        spdlog::error("gid {} not found in any tileset.", gid);
        return engine::component::TileInfo();
    } 
    ...
    if (file_path.empty()) {
        spdlog::error("Tileset file '{}' missing 'file_path' property.", tileset_it->first);
        return engine::component::TileInfo();
    }
    // 图块集分为两种情况，需要分别考虑
    if (tileset.contains("image")) {    // 这是单一图片的情况
        ...
        if (!tileset.contains("tiles")) {   // 没有tiles字段的话不符合数据格式要求，直接返回空的瓦片信息
            spdlog::error("Tileset file '{}' missing 'tiles' property.", tileset_it->first);
            return engine::component::TileInfo();
        }
        ...
        for (const auto& tile_json : tiles_json) {
            ...
            if (tile_id == local_id) {   // 找到对应的瓦片，进行后续操作
                if (!tile_json.contains("image")) {   // 没有image字段的话不符合数据格式要求，直接返回空的瓦片信息
                    spdlog::error("Tileset file '{}' tile {} missing 'image' property.", tileset_it->first, tile_id);
                    return engine::component::TileInfo();
                }
    ...
    spdlog::error("Tileset file '{}' not found gid {} tile.", tileset_it->first, gid);
    return engine::component::TileInfo();
}
...
void LevelLoader::loadTileset(std::string_view tileset_path, int first_gid)
{
    ...
    try {
        ...
        spdlog::error("Failed to parse Tileset JSON file '{}': {} (at byte {})", tileset_path, e.what(), e.byte);
        return;
    }
    ts_json["file_path"] = tileset_path;    // 将文件路径存储到json中，后续解析图片路径时需要
    tileset_data_[first_gid] = std::move(ts_json);
    spdlog::info("Tileset file '{}' loaded, firstgid: {}", tileset_path, first_gid);
}
...
std::string LevelLoader::resolvePath(std::string_view relative_path, std::string_view file_path)
{
    ...
    return final_path.string();
        ...
        spdlog::error("Failed to resolve path '{}': {}", relative_path, e.what());
