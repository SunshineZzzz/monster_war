...
SceneManager::~SceneManager() {
    spdlog::trace("scene manager destroyed successfully.");
...
void SceneManager::close() {
    spdlog::trace("scene manager closed successfully.");
    // 清理栈中所有剩余的场景（从顶到底）
    while (!scene_stack_.empty()) {
        if (scene_stack_.back()) {
            spdlog::debug("scene '{}' cleaned successfully.", scene_stack_.back()->getName());
...
void SceneManager::pushScene(std::unique_ptr<Scene>&& scene) {
    if (!scene) {
        spdlog::warn("attempt to push null scene pointer to scene manager.");
        return;
    }
    spdlog::debug("scene '{}' pushed successfully to scene manager.", scene->getName());
...
void SceneManager::popScene() {
    if (scene_stack_.empty()) {
        spdlog::warn("attempt to pop scene from empty scene stack.");
        return;
    }
    spdlog::debug("scene '{}' popped successfully from scene manager.", scene_stack_.back()->getName());
...
void SceneManager::replaceScene(std::unique_ptr<Scene>&& scene) {
    if (!scene) {
        spdlog::warn("attempt to replace scene with null scene pointer.");
        return;
    }
    spdlog::debug("scene '{}' replaced successfully with scene '{}' in scene manager.", scene->getName(), scene_stack_.back()->getName());
